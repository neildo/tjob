...
$ mkdir -p /sys/fs/cgroup/job_id/jail
...
$ echo "20000 100000" > /sys/fs/cgroup/job_id/jail/cpu.max

# limit memory to 20M
$ echo "20M" > /sys/fs/cgroup/job_id/jail/memory.max
...
func run() {
   ...
   cgroup, err := os.OpenFile("/sys/fs/cgroup/job_id/jail", os.O_RDONLY, 0)
	must(err)
...
```
2. Open the file descriptor for the `/sys/fs/cgroup/job_id/jail` cgroup with [limits on CPU, Memory, and IO](#control-group-interface-files).
```golang
   cgroup, err := os.OpenFile("/sys/fs/cgroup/job_id/jail", os.O_RDONLY, 0)
```
